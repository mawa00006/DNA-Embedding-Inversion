"""Generate DNA embeddings from BioFM-265M using sequences from a file.

This script extracts embeddings from a BioFM checkpoint (a Mistral-architecture
causal decoder) for sequences provided in a file. The output is HDF5 files for
train, val, and test with datasets 'sequences' and 'embeddings'.
...
* UNVERIFIED / highest risk in this file: BioFM's documented inference path
  (the model card) uses a separate `biofm-eval` package's `AnnotatedModel` /
  `AnnotationTokenizer`, built around "BioToken" -- annotated special tokens
  plus nucleotide tokens carrying variant/structural context. The HF repo
  itself ships no standard tokenizer files (no tokenizer.json, vocab.txt, or
  merges.txt; tokenizer_config.json contains only `char_ords` and
  `model_max_length`, which look like raw material for that external
  package's tokenizer, not a `transformers`-loadable one). Whether
  `AutoTokenizer.from_pretrained(checkpoint)` produces any usable tokenizer at
  all for this checkpoint is therefore unverified -- it may fail to load, or
  load something that does not match BioFM's intended input format. This
  cannot be resolved without loading the model; treat this generator as
  unverified until the cluster probe confirms `AutoTokenizer` returns a usable
  tokenizer and the model actually runs the "special tokens + nucleotide"
  scheme with `add_special_tokens=False`, i.e. plain-nucleotide input with no
  BioToken annotation. If that annotation is load-bearing for representation
  quality, forcing `add_special_tokens=False` (required for comparability with
  every other model in this benchmark) may produce degraded embeddings for
  this checkpoint specifically -- reported, not silently fixed.
* config.pad_token_id (9) / bos_token_id (11) / eos_token_id (10) are set in
  config.json, so pad_token is expected to already be defined once a tokenizer
  loads; the pad_token-is-None guard below is kept anyway since it is a
  general decoder-only-checkpoint failure mode and costs nothing to guard.
* AutoModel loads the bare Mistral backbone against MistralForCausalLM
  weights (lm_head dropped); missing_keys is asserted empty so a mismatch
  fails loudly instead of silently random-initialising part of the model.
* tokenizer.model_max_length is meaningful here (6144, matching the model
  card's documented 6k training context) -- unlike the other two models in
  this batch, this is not a sentinel and is used directly.
...
Example usage:
    python generate_biofm_embeddings.py input_path=data.csv \
        checkpoint=m42-health/BioFM-265M
"""
...
def embed_batch_biofm(
    ...
    """Tokenize a batch of DNA sequences with the BioFM tokenizer and extract embeddings.

    Tokenizes without special tokens so no BioToken annotation is injected --
    see the UNVERIFIED note at module level about whether that is load-bearing
    for this checkpoint. Returns a list of NumPy arrays, each of shape
...
def _load_biofm(checkpoint: str, revision: Optional[str], device: str):
    """Load the BioFM tokenizer/model and return ``(tokenizer, model, model_max)``.

    See the module-level UNVERIFIED note: this checkpoint's repo ships no
    standard tokenizer files, so this call may fail on the cluster in a way it
    cannot fail here without network access to the checkpoint.
    """
    logger = logging.getLogger(__name__)
    logger.info(f"Loading BioFM tokenizer/model from: {checkpoint} (revision={revision})")
    ...
    assert tokenizer.pad_token is not None, "BioFM tokenizer has no pad token"
...
def _make_embed_chunk_biofm(
    ...
    Sub-batches by ``batch_size`` and reuses ``embed_batch_biofm``; the model is
    ...
    """
    logger = logging.getLogger(__name__)
    tokenizer, model, model_max = _load_biofm(checkpoint, revision, device)
...
@hydra.main(config_path="../conf", config_name="generate/biofm", version_base=None)
def main(cfg: DictConfig) -> None:
    """Hydra entrypoint: generate sequences and extract BioFM embeddings."""
    logger = logging.getLogger(__name__)
    logger.info("Generating DNA sequences and BioFM embeddings...")
    ...
    if run_generation(
        ...
        make_embed_chunk=lambda: _make_embed_chunk_biofm(
            checkpoint,
            device,
            int(cfg.seq_length),
            int(batch_size),
