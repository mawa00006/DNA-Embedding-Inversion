"""Generate DNA embeddings from DNABERT-1 using sequences from a file.

This script extracts embeddings from a DNABERT-1 checkpoint (a BERT encoder
over overlapping 6-mer tokens) for sequences provided in a file. The output is
HDF5 files for train, val, and test with datasets 'sequences' and 'embeddings'.
...
* The tokenizer's vocabulary is exactly the 4**6 overlapping 6-mers (plus 5
  special tokens): it is a slow `BertTokenizer` that treats whitespace-split
  words as pre-tokenized units, not raw characters. Feeding a raw ACGT string
  makes the whole string one out-of-vocabulary token. Sequences must first be
  converted to whitespace-joined overlapping 6-mers (`_to_kmers` below); an
  L-nt sequence then yields L-5 tokens, not L. This changes nothing for this
  pipeline (mean-pooling is invariant to token count) but is easy to miss.
* config.json declares `auto_map` for both `AutoConfig` and `AutoModel`,
  pointing at the checkpoint's own `configuration_bert.BertConfig` and
  `dnabert_layer.BertModel`. Both are one-line pass-through subclasses
  (`super().__init__(config)`) of transformers' own `BertConfig`/`BertModel`,
  so behavior is identical to stock BERT -- `trust_remote_code=True` is
  required to resolve the classes at all, but changes no computation.
  `AutoModel` here is the bare backbone (the masked-LM checkpoint minus its
  MLM head), unlike GENA-LM's checkpoint.
* tokenizer_config.json uses the legacy `max_len` key, which modern
  `transformers` does not read, so `tokenizer.model_max_length` silently falls
  back to a ~1e30 sentinel. `config.max_position_embeddings` (512) is the real
  bound; `_resolve_model_max` uses it whenever the tokenizer value looks like
  the sentinel.
...
Example usage:
    python generate_dnabert1_embeddings.py input_path=data.csv \
...
# The checkpoint's vocabulary is the overlapping 6-mers of ACGT.
KMER = 6
...
def _to_kmers(sequence: str, k: int = KMER) -> str:
    """Convert a raw ACGT string to whitespace-separated overlapping k-mers.

    DNABERT-1's slow BertTokenizer treats whitespace-split words as
    pre-tokenized vocabulary entries; without this conversion the whole
    sequence is a single out-of-vocabulary token. An L-nt sequence yields
    L-k+1 tokens.
    """
    assert len(sequence) >= k, f"sequence shorter than k={k}: {sequence!r}"
...
def _resolve_model_max(tokenizer, config) -> int:
    ...
    ``tokenizer.model_max_length`` is unreliable here: tokenizer_config.json
    uses the legacy `max_len` key, which transformers does not read, so it
    silently falls back to the ~1e30 sentinel. ``config.max_position_embeddings``
    is the real, architecture-level bound.
    """
...
def embed_batch_dnabert1(
    ...
    """Tokenize a batch of DNA sequences as overlapping 6-mers and extract per-token embeddings.

    Tokenizes without special tokens so embeddings align 1:1 with k-mer tokens
    (not with input nucleotides -- see module docstring). Returns a list of
    NumPy arrays, each of shape (num_tokens_i, embedding_dim).
    """
    assert sequences, "Empty batch"
    kmer_sequences = [_to_kmers(s) for s in sequences]
    enc = tokenizer(
        kmer_sequences,
        return_tensors="pt",
        padding=True,
...
def _load_dnabert1(checkpoint: str, revision: Optional[str], device: str):
    """Load the DNABERT-1 tokenizer/model and return ``(tokenizer, model, model_max)``."""
    logger = logging.getLogger(__name__)
    logger.info(f"Loading DNABERT-1 tokenizer/model from: {checkpoint} (revision={revision})")
    ...
    assert tokenizer.pad_token is not None, "DNABERT-1 tokenizer has no pad token"
...
def _make_embed_chunk_dnabert1(
    checkpoint: str,
    device: str,
    seq_length: int,
    batch_size: int,
    ...
    Sub-batches by ``batch_size`` and reuses ``embed_batch_dnabert1``; the
    ...
    """
    logger = logging.getLogger(__name__)
    tokenizer, model, model_max = _load_dnabert1(checkpoint, revision, device)

    # seq_length is nucleotides; after k-mer conversion the token count is
    # seq_length - KMER + 1, always <= seq_length, so this bound is
    ...
    probe_kmers = _to_kmers("ACGTACGT")
    with torch.no_grad():
        probe_enc = tokenizer([probe_kmers], return_tensors="pt", add_special_tokens=False)
        probe = model(
...
@hydra.main(config_path="../conf", config_name="generate/dnabert1", version_base=None)
def main(cfg: DictConfig) -> None:
    """Hydra entrypoint: generate sequences and extract DNABERT-1 embeddings."""
    logger = logging.getLogger(__name__)
    logger.info("Generating DNA sequences and DNABERT-1 embeddings...")
    ...
    if run_generation(
        ...
        make_embed_chunk=lambda: _make_embed_chunk_dnabert1(
            checkpoint,
            device,
            int(cfg.seq_length),
            int(batch_size),
